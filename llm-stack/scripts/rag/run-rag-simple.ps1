# LLM Stack RAG System - Simplified Version
...
function Show-Help {
    ...
    Write-Host "  .\run-rag-simple.ps1 -Action build-index -InputFile ../export/exports/sample-combined-20250830-234124.jsonl" -ForegroundColor White
    Write-Host "  .\run-rag-simple.ps1 -Action search -Query 'sample document'" -ForegroundColor White
    Write-Host "  .\run-rag-simple.ps1 -Action web -Port 5000" -ForegroundColor White
    Write-Host "  .\run-rag-simple.ps1 -Action test" -ForegroundColor White
    Write-Host ""
...
function Build-RAGIndex {
    if (-not $InputFile) {
        ...
        Write-Host "Use: .\run-rag-simple.ps1 -Action build-index -InputFile path" -ForegroundColor Yellow
        exit 1
    }
...
function Search-RAGIndex {
    if (-not $Query) {
        ...
        Write-Host "Use: .\run-rag-simple.ps1 -Action search -Query 'your search query'" -ForegroundColor Yellow
        exit 1
    }
...
function Test-RAGSystem {
    ...
    if (Test-Path $OutputDir) {
        ...
        try {
            python rag-search.py --index-dir $OutputDir --query "test" --n-results 3
        ...
        Write-Host "Build the index first with: .\run-rag-simple.ps1 -Action build-index -InputFile path" -ForegroundColor Yellow
    }
}
